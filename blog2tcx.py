...
for item in url.entries:
    if re.search(r'^(Machelen|Berbroek)', item.title):
        ...
        if match is not None:
            runningTime=int(match.group(1))*3600+int(match.group(2))*60+int(match.group(3))
        else:
            ...
            if match is not None:
                ...
                print(content)
                exit(1)
        filename = "run-%s.tcx" % (item.published)
        print(filename)
        ...
        if os.path.isfile(filename):
            ...
            exit(1)
        f = open(filename, "w")
        f.write('<?xml version="1.0" encoding="UTF-8"?><TrainingCenterDatabase xsi:schemaLocation="http://www.garmin.com/xmlschemas/TrainingCenterDatabase/v2 http://www.garmin.com/xmlschemas/TrainingCenterDatabasev2.xsd" xmlns="http://www.garmin.com/xmlschemas/TrainingCenterDatabase/v2" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:ns3="http://www.garmin.com/xmlschemas/ActivityExtension/v2">')
        f.write("\n")
        f.write('<Activities>'+"\n")
